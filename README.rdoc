...
config ディレクトリに config.yml を作成し、データセットが置かれているディレクトリパスを指定します。(データセットすべてを対象とするのは膨大な時間がかかるため、試しに実行する場合には少数のXMLファイルを入れたディレクトリを指定してください)

 # config/config.yml
 datadir: "/var/opt/PubMedDataSet"
